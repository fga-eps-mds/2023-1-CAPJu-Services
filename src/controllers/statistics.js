...
export class StatisticsController {
    ...
    getProcessByStepInFlow = async (req, res) => {
        try {
            ...
            if (!idFlow)
                return res.status(412).json({ error: "É necessário fornecer um id de um fluxo" })

            const stages = {}

            // Pega os processos do fluxo
            const processesbyIdFlow = await this.processService.getProcessByIdFlow(idFlow);
            ...
            for(const item of processesbyIdFlow){
                ...
                stages[key ?? "nao_iniciado"].countProcess = stages[key ?? "nao_iniciado"].process.length
            }
